...
const WALLET_BATCH_SIZE = 30; // Follow Foundry script pattern
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const abxContract = await ethers.getContractAt(
    "Abx",
    abx.address
  );
  ...
  const airdropBalance = await abxContract.balanceOf(airdropDistributor.address);
  log(`Total airdrop amount: ${ethers.formatEther(totalAmount.toString())} ABX`);
  log(`AirdropDistributor balance: ${ethers.formatEther(airdropBalance.toString())} ABX`);
  ...
  if (totalAmount > airdropBalance) {
    ...
    log(`Adjusted first airdrop amount by -${ethers.formatEther(excess.toString())} ABX to fit balance`);
  }
    ...
    try {
      ...
      const distributeTx = await airdropDistributorContract
        ...
        .distributeTokens(batchWallets, batchAmounts, {
          gasLimit: 10000000, // Set high gas limit for batch operations
        });
      ...
      // Log batch details
      for (let j = 0; j < batchLen; j++) {
        log(`  ${batchWallets[j]}: ${ethers.formatEther(batchAmounts[j])} ABX`);
      }
  ...
  log(`Total recipients: ${walletsLength}`);
  log(`Total amount distributed: ${ethers.formatEther(totalAmount.toString())} ABX`);
