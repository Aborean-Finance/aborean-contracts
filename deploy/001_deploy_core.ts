...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  // Deploy ABX token first
  log("\n=== DEPLOYING CORE CONTRACTS ===");
  const abxDeployment = await deploy("Abx", {
    from: deployer,
    log: true,
    waitConfirmations: 1,
  });
  ...
  log("Deploying PoolFactory (zkSync compatible)...");
  ...
  // 5. Set minter for ABX token
  log("Setting minter for ABX token...");
  await execute(
    "Abx",
    { from: deployer, log: true },
    "setMinter",
  ...
  const whitelistTokens = [...config.whitelistTokens, abxDeployment.address];
  await execute(
    "Voter",
    { from: deployer, log: true },
    "initialize",
  ...
  log(`ABX Token: ${abxDeployment.address}`);
