...
contract Minter is IMinter {
    ...
    function initialize(AirdropParams memory params) external {
        ...
        for (uint256 i = 0; i < _len; i++) {
            abx.mint(params.liquidWallets[i], params.liquidAmounts[i]);
    ...
    function calculateGrowth(uint256 _minted) public view returns (uint256 _growth) {
        ...
        uint256 _abxTotal = abx.totalSupply();

        return (((_minted * (_abxTotal - _veTotal)) / _abxTotal) * (_abxTotal - _veTotal)) / _abxTotal / 2;
    }
    ...
    function updatePeriod() external returns (uint256 _period) {
        ...
        if (block.timestamp >= _period + WEEK) {
            ...
            uint256 _emission;
            uint256 _totalSupply = abx.totalSupply();
            ...
            uint256 _balanceOf = abx.balanceOf(address(this));
            if (_balanceOf < _required) {
                abx.mint(address(this), _required - _balanceOf);
            }

            abx.safeTransfer(address(team), _teamEmissions);
            abx.safeTransfer(address(rewardsDistributor), _growth);
            rewardsDistributor.checkpointToken(); // checkpoint token balance that was just minted in rewards distributor

            abx.safeApprove(address(voter), _emission);
            voter.notifyRewardAmount(_emission);

            emit Mint(msg.sender, _emission, abx.totalSupply(), _tail);
        }
    }
}
